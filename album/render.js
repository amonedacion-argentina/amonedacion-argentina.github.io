...
function actualizarEstadisticas() {
    ...
    // Solo intenta aplicar traducciones si i18next está disponible
    if (typeof i18next !== 'undefined') {
        i18next.changeLanguage(App.estado.idioma);
    }
}
...
function generarHTMLStats(totalNFTs, nftsPropios, categoriasStats) {
    ...
    // Usa las traducciones del estado si están disponibles
    const i18n = App.estado.i18n || {};
    const catTranslations = i18n.cat || {};

    const itemsCategorias = categoriasStats.map(cat => {
        // Usa la traducción o el nombre como fallback
        const nombreMostrar = catTranslations[cat.nombre] || cat.nombre;
        
        return `
            <li>
                <span class="categoria-nombre" data-i18n="cat.${cat.nombre}">${nombreMostrar}</span>
    ...
    return `
        <div class="estadisticas-global">
            <h3 data-i18n="stats.mi-coleccion">${i18n.stats?.['mi-coleccion'] || 'Mi Colección'}</h3>
        ...
        <div class="estadisticas-categorias">
            <h4 data-i18n="stats.por-categoria">${i18n.stats?.['por-categoria'] || 'Por Categoría:'}</h4>
